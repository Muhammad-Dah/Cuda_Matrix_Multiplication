...
void run(int N, int dim1, int dim2)
{
	...
	Left_matrix_x = Left_matrix_y = Right_matrix_x = Right_matrix_y = N;
    ...
    dim3 Block_dim(dim1, dim2);
    //Grid dimension is found by dividing matrix dimension to block_size
	dim3 Grid_dim((N  + Block_dim.x - 1) / Block_dim.x,
                 (N  + Block_dim.y - 1) / Block_dim.y);
	
	int numThreadsPerBlock = Block_dim.x * Block_dim.y;
	int numThreadsBlock = Grid_dim.x * Grid_dim.y;
	
	
	printf("CUDA kernel launch with %d blocks of %d threads\n", numThreadsBlock, numThreadsPerBlock);
...
int main( int argc, char * argv [] ) {
	...
	for( int i = 0; i < argc; ++i ) {
        if(strcmp(argv[ i ], "-N")*strcmp(argv[ i ], "--N") == 0){
				N = atoi(argv[i+1]);
				// printf( "N  = %d\n",N);
		}
		if(strcmp(argv[ i ], "-dim")*strcmp(argv[ i ], "--dim") == 0){
				a = atoi(argv[i+1]);
				b = atoi(argv[i+2]);
				// printf( "dim  = (%d,%d)\n", a, b);
		}
